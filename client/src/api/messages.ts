...
export async function createMessage(message: MessageType, userId: string) {
	try {
		const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/messages`, {
			message,
			userId,
		});
...
export async function fetchMessages(pageParam: any | undefined = undefined, studyGroupId: string) {
	try {
		...
		const response = await axios.get(
			`${import.meta.env.VITE_API_BASE_URL}/api/messages${queryParams}`
		);
		return response.data;
	} catch (error) {
...
export async function fetchMessagesWithFiles(
	pageParam: any | undefined = undefined,
	studyGroupId: string
) {
	try {
		let queryParams = "?";
		...
		const response = await axios.get(
			`${import.meta.env.VITE_API_BASE_URL}/api/messages/files${queryParams}`
		);
		return response.data;
	} catch (error) {
		throw new Error("Failed to fetch messages with files!");
	}
}
